...
// File    : top_multichannel.v
// Module  : top_multichannel
// Created : 2026-06-27
// Revised : 2026-09-17
...
// Project 1 v7: four on-chip XADC channels plus v6's UART/I2C functions.
module top_multichannel (
    ...
    wire iic_start = iic_request && !iic_busy && !eeprom_inflight;
    ...
    wire frame_start = send_state == S_IDLE &&
        (iic_reply_valid || pending || (streaming && sample_count >= 16));
    ...
    always @(posedge clkin_50m) begin
        if (rst) begin
            ...
            if (request_valid && !iic_start) begin
                pending <= 1;
            ...
            case (send_state)
                S_IDLE: if (iic_reply_valid) begin
