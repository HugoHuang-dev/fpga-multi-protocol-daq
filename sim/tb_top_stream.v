...
module tb_top_stream;
    ...
    reg [15:0] crc;
    integer n=0, i, j, k;
    top_stream dut(.clkin_50m(clk), .rst_n(rst_n),
        .uart_rxd(uart_rxd), .uart_txd(uart_txd), .led0(led0));
    task send_byte;
        input [7:0] v;
        integer bit_no;
        begin
    ...
    initial begin
        ...
        wait(n>=49);
        if(got[8]!==8'hA5 || got[9]!==8'h5A || got[10]!==8'h90 ||
           got[11]!==0 || got[12]!==34 || got[13]!==0 || got[14]!==0)
            $fatal(1,"batch header wrong");
        for(j=0;j<16;j=j+1)
            if(got[15+j*2]!==8'h09 || got[16+j*2]!==8'h77)
                $fatal(1,"sample %0d wrong: %02x %02x",j,
                    got[15+j*2],got[16+j*2]);
        crc=16'hFFFF;
        ...
        if(got[47]!==crc[7:0] || got[48]!==crc[15:8])
            $fatal(1,"batch CRC wrong: got %02x %02x expected %04x",
                got[47],got[48],crc);
        wait(n>=90);
        if(got[49]!==8'hA5 || got[50]!==8'h5A || got[51]!==8'h90 ||
           got[52]!==1 || got[53]!==34)
            $fatal(1,"second batch sequence wrong");
        for(j=0;j<16;j=j+1)
            if(got[56+j*2]!==8'h09 || got[57+j*2]!==8'h77)
                $fatal(1,"second batch sample %0d wrong",j);
        crc=16'hFFFF;
